 ...
 * @author Gaurav Jain
 */
class OSInvoiceModelBuyer extends OSInvoiceModel
{
	protected $recordId ;
    ...
    protected function _buildQueryFrom(Rb_Query &$query)
    {
    	...
    	$join = ' `#__osinvoice_buyer` AS t  ON ( t.`buyer_id` = joomlausertbl.`id` ) ';
	...
	protected function _buildQueryFilter(Rb_Query &$query, $key, $value)
    {
    	...
    	JLog::add(is_array($value), Rb_Text::_('COM_OSINVOICE_VALUE_FOR_FILTERS_MUST_BE_AN_ARRAY'));
...
class OSInvoiceModelformBuyer extends OSInvoiceModelform {
